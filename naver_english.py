...
def init():
    ...
    # 폴더 정리
    if not os.path.exists(f"./{targetMm}"): os.mkdir(f"./{targetMm}")
    if os.path.exists(f"./{targetMm}/{targetDd}"): shutil.rmtree(f"./{targetMm}/{targetDd}")
    os.mkdir(f"./{targetMm}/{targetDd}")
    os.chdir(f"./{targetMm}/{targetDd}")
...
if __name__ == '__main__':
    ...
    async def main():
        ...
        await client.send_file(-1002377578524, f"{targetMm}/{resultFile}", parse_mode="html")
